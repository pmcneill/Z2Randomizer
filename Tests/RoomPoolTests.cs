...
public class RoomPoolTests
{
    ...
    #region DetermineRoomVariants

    [TestMethod]
    public void DetermineRoomVariants_RemovesDuplicatesFromGroup()
    {
        var room1 = CreateMockRoom("Dup1", RoomGroup.VANILLA, palaceNumber: 1, duplicateGroup: "GroupA");
        var room2 = CreateMockRoom("Dup2", RoomGroup.VANILLA, palaceNumber: 1, duplicateGroup: "GroupA");
        var room3 = CreateMockRoom("Dup3", RoomGroup.VANILLA, palaceNumber: 1, duplicateGroup: "GroupA");
        var normalRoom = CreateMockRoom("Normal1", RoomGroup.VANILLA, palaceNumber: 1);
        var bossRoom = CreateMockRoom("Boss1", RoomGroup.VANILLA, isBossRoom: true, palaceNumber: 1, hasDownExit: true);

        var mockPalaceRooms = CreateMockPalaceRooms([room1, room2, room3, normalRoom, bossRoom]);
        var props = CreateMockProps(allowVanilla: true);
        var pool = new RoomPool(mockPalaceRooms, 1, props);

        pool.NormalRooms.Should().HaveCount(4);
        pool.DetermineRoomVariants(new Random(42));
        pool.NormalRooms.Should().HaveCount(2);
        pool.NormalRooms.Should().Contain(r => r.Name == "Normal1");
        pool.NormalRooms.Find(r => r.DuplicateGroup == "GroupA" && pool.NormalRooms.Contains(r)).Should().NotBeNull();
    }
